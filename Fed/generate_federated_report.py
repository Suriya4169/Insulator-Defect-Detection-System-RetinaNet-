...
def generate_report():
    ...
    full_dataset = IDIDDataset(DATA_ROOT, ANNOTATION_FILE, transforms=get_transform(train=False), target_size=(800, 800))
    ...
    with PdfPages(OUTPUT_REPORT) as pdf:
        ...
        plt.axis('off')
        plt.text(0.5, 0.90, "AI Model Evaluation Report (Federated Learning)", ha='center', fontsize=24, weight='bold', color='#2c3e50')
        ...
        --------------------------------------------------------
        Mean Average Precision (mAP @ 0.50):           {map_50:.4f}
        Mean Average Precision (mAP @ 0.75):           {map_75:.4f}
        
        Class-wise Breakdown (AP @ 0.50)
        --------------------------------------------------------
        """
