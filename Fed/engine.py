...
def save_preview(images, targets, outputs, class_names, output_dir, batch_idx):
    """Save visual previews of predictions vs ground truth"""
    os.makedirs(os.path.join(output_dir, 'previews'), exist_ok=True)
    
    # Process up to 4 images from the batch
    for idx in range(min(len(images), 4)):
        img = images[idx].cpu().permute(1, 2, 0).numpy()
        # Denormalize if needed (assuming standard ImageNet stats were not used, just ToTensor)
        # If ToTensor was used, range is [0, 1], which mpl accepts.
        ...
        for box, label in zip(gt_boxes, gt_labels):
            ...
            ax.text(x, y, f"GT: {name}", color='white', fontsize=8, backgroundcolor='green')
            
        # Predictions - Red
        pred_boxes = outputs[idx]['boxes'].cpu().detach().numpy()
        pred_labels = outputs[idx]['labels'].cpu().detach().numpy()
        pred_scores = outputs[idx]['scores'].cpu().detach().numpy()
        ...
        for box, label, score in zip(pred_boxes, pred_labels, pred_scores):
            if score > 0.5: # Only show confident predictions
                ...
                ax.add_patch(rect)
                name = class_names.get(int(label), str(label))
                ax.text(x, y+15, f"{name}: {score:.2f}", color='white', fontsize=8, backgroundcolor='red')
                
        plt.axis('off')
        plt.savefig(os.path.join(output_dir, 'previews', f'val_batch{batch_idx}_img{idx}.png'))
        plt.close(fig)
...
@torch.no_grad()
def evaluate(model, data_loader, device, score_threshold=0.05, iou_thresholds=None, output_dir=None, class_names=None):
    """
    ...
    for images, targets in pbar:
        ...
        # Save Preview for first batch if output_dir provided
        if output_dir is not None and batch_counter == 0 and class_names is not None:
             save_preview(images, targets, outputs, class_names, output_dir, batch_counter)
        batch_counter += 1
